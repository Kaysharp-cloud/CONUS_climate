...
OUTPUT_DIR = BASE_DIR / "outputs" / "noaa_raw_18k"

df_weather_info = pd.read_csv(DATA_DIR / "All_18kstations_details.csv")
station_ids = df_weather_info.ID.unique()
...
def check_downloaded_ids():
    ids_downloaded = glob.glob(f"{output_dir}/*csv")
    ids_downloaded_clean = [ids.replace(output_dir,"") for ids in ids_downloaded]
    ids_downloaded_clean = [ids.replace(".csv","") for ids in ids_downloaded_clean]
    return ids_downloaded_clean


downloaded_ids = check_downloaded_ids()
filtered_ids = [x for x in station_ids if x not in downloaded_ids ]
...
def create_dataframe(element, dict_element):
    ...
    df_element[df_element.columns[4]] = df_element[df_element.columns[4]].astype(float)
    return df_element
...
stations_downloaded = []
def dly_to_csv(ftp, station_ids):
    ...
    for station_id in station_ids:
        ...
        stations_downloaded.append(station_id)
def download_all_stations(id):
    try:
...
filtered_ids = [x for x in station_ids if x not in downloaded_ids ]
len(filtered_ids)
filtered_ids_copy = filtered_ids.copy()
filtered_ids_copy= filtered_ids_copy[:]
problematic_ids =[]
while filtered_ids_copy:

    for i, ids in enumerate(filtered_ids_copy[:]):   # iterate on a copy
        try:
            download_all_stations(ids)

            # remove upon success
            filtered_ids_copy.remove(ids)
            print(f"{i}th_downloaded")

        except TimeoutError:
            problematic_ids.append(ids)
            print(f"The number of problematic is {len(problematic_ids)}")
            time.sleep(180)
            continue
print("saved all stations sucessfully")
